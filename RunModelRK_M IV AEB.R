...
# load data
# source("PrepDataRK_M.R") # aeb note - commented
...
# AEB note - copying these functions here as I'm not running the prep data script
...
# AEB note - also needed to add this here
n.inds <- length(unique(eh$ID))
###
n.inds <- n.inds - length(unkAge)
...
code <- nimbleCode({
  ...
  for (i in 1:n.inds){
    for (t in (first[i]+1):n.occasions){
      ...
      #### Likelihood ####
      # AEB - trying this index change to t-1 in the last index
  ...
  for (i in 1:n.inds){
    for (t in first[i]:(n.occasions-1)){
      ...
      # Survival
      phi[i,t] <- mean.phi[ageC[age[i,t]], t]
  ...
  for (i in 1:n.inds){
    for (t in (first[i]+1):n.occasions){
      ...
      # AEB change - moved this to the observation matrix section
...
# AEB checking that z[, first] is known for all inds...
for (ii in 1:n.inds) {
  print(z_dat[ii, first[ii]])
}
# mostly seen on site
# but a handful dead by natural causes (z = 4) - is that expected?
# also a small number of dead by roadkill
# are these inds that died within their first season (but not first obs)?
