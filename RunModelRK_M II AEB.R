...
# last minute simulations
hist(plogis(rnorm(1000, 1, 0.75)))
hist(rnorm(1000, 1, 0.75))
...
code <- nimbleCode({
  ...
  for (i in 1:n.inds){
    for (t in (first[i]+1):n.occasions){
      ...
      # AEB note - index order is wrong above
      #trans.mat[i,1,3,t]
      #obs.mat[i,1,2,t]
      ...
      # AEB note - do you want index t on left and t-1 on right?
      phi[i,t] <- mean.phi[ageC[age[i,t-1]], t-1]
...
inits <- list(
  z        = z_inits,
  mu.juv   = rbeta(1, 8, 2), # AEB note - make sure these are consistent with priors
  mu.sub   = rbeta(1, 8, 2),
  mu.pri   = rbeta(1, 8, 2),
  mu.pre   = rbeta(1, 8, 2),
  mu.sen   = rbeta(1, 8, 2),
...
# AEB note - useConjugacy false speeds things along 
conf <- configureMCMC(Rmodel, monitors = params, thin = nt, useConjugacy = F) # slow
